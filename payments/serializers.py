...
class AvailablePlansSerializer(serializers.Serializer):
    ...
    price_monthly = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Preço mensal em euros")
    features = serializers.ListField(
        child=serializers.CharField(),
        help_text="Lista de funcionalidades do plano"
    )
    credits_included = serializers.IntegerField(help_text="Créditos de comunicação inclusos")
    is_current = serializers.BooleanField(help_text="Se é o plano atual do usuário")
    can_upgrade = serializers.BooleanField(help_text="Se pode fazer upgrade para este plano")
...
class CurrentSubscriptionSerializer(serializers.Serializer):
    ...
    cancel_at_period_end = serializers.BooleanField(help_text="Se será cancelada no fim do período")
    next_billing_date = serializers.DateTimeField(help_text="Próxima data de cobrança", allow_null=True)
    price_monthly = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Preço mensal")
...
class PaymentHistoryItemSerializer(serializers.Serializer):
    ...
    type = serializers.ChoiceField(
        choices=['subscription', 'credit_purchase'],
        help_text="Tipo de transação"
    )
    description = serializers.CharField(help_text="Descrição da transação")
    amount = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Valor em euros")
...
class PaymentHistorySerializer(serializers.Serializer):
    ...
    transactions = PaymentHistoryItemSerializer(many=True, help_text="Lista de transações")
    total_spent = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Total gasto")
    subscription_total = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Total em assinaturas")
    credits_total = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Total em créditos")
...
class BillingOverviewSerializer(serializers.Serializer):
    ...
    credit_balance = serializers.DecimalField(max_digits=10, decimal_places=2, help_text="Saldo atual de créditos")
    can_purchase_credits = serializers.BooleanField(help_text="Se pode comprar créditos extras")
    has_auto_renewal = serializers.BooleanField(help_text="Se tem renovação automática de créditos")
...
class SubscriptionActionSerializer(serializers.Serializer):
    ...
    action = serializers.ChoiceField(
        choices=['cancel', 'reactivate'],
        help_text="Ação a ser executada na assinatura"
    )
