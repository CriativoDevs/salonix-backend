...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            professionals = self._create_professionals(tenant, admin_user, professionals_count)
            ...
            slots = self._create_schedule_slots(tenant, professionals, days_back, batch_size)
    ...
    def _create_services(self, tenant, admin_user, count):
        ...
        services = []
        existing_services = set(Service.objects.filter(tenant=tenant).values_list('name', flat=True))

        for i in range(count):
            base_service = random.choice(service_types)
            name = f"{base_service[0]} {i+1}" if i >= len(service_types) else base_service[0]
    ...
    def _create_customers(self, tenant, count, batch_size):
        ...
        existing_emails = set(SalonCustomer.objects.filter(tenant=tenant).values_list('email', flat=True))
        ...
        for i in range(count):
            ...
            if len(batch) >= batch_size:
                created_customers = SalonCustomer.objects.bulk_create(batch, ignore_conflicts=True)
                    ...
                    if random.random() > 0.2:  # 80% chance de trabalhar
                        for hour in working_hours:
                            ...
                            if random.random() > 0.3:  # 70% chance de ter slot
                                ...
                                if len(batch) >= batch_size:
                                    created_slots = ScheduleSlot.objects.bulk_create(batch, ignore_conflicts=True)
    ...
    def _create_appointments(self, tenant, customers, services, slots, count, batch_size):
        """Cria agendamentos distribuídos no tempo"""
        appointments = []
        batch = []
        
        # Pegar slots disponíveis
        available_slots = list(ScheduleSlot.objects.filter(
            tenant=tenant,
            is_available=True
        ...
        client_users = []
        for i in range(min(50, count // 10 + 1)):  # Criar alguns usuários para reutilizar
        ...
        for i in range(min(count, len(available_slots))):
            ...
            if len(batch) >= batch_size:
                ...
                slot_ids = [app.slot_id for app in batch if app.status in ['scheduled', 'confirmed', 'completed']]
